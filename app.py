...
def ensure_reduced_scaler():
    ...
    selected_features = ['HBsAgbaseline', 'ALT12w_HBsAg12w']
...
def load_model_and_preprocessors():
    ...
    numeric_features = ['HBsAgbaseline', 'ALT12w_HBsAg12w']
...
def prepare_input_data(baseline_hbsag, week12_hbsag, week24_hbsag, week12_alt):
    ...
    hbsag24wdecline_1 = 1 if np.log10(baseline_hbsag / week24_hbsag) >= 1 else 0
...
with st.container():
    ...
    with col3:
        week24_hbsag = st.number_input("Week 24 HBsAg (IU/mL)", 0.0, 25000.0, 10.0, 1.0)
        st.caption("ℹ️ Enter 0.05 if ≤ 0.05; system internally converts to 0.01")
...
if st.button("Predict"):
    ...
    hbsag24wdecline_1 = display_df['HBsAg24wdecline_1'].values[0]

    st.subheader("Prediction Result")
    col1, col2 = st.columns([1, 1])
    with col1:
        st.metric("HBsAg Clearance Probability at 48w", f"{prediction:.1%}")
        if prediction < 0.3:
    ...
    with col2:
        ...
        st.table(pd.DataFrame({
            "Feature": [
                "Baseline HBsAg", "Week 12 HBsAg", "Week 24 HBsAg", 
                "Week 12 ALT", "ALT / HBsAg12w", "HBsAg decline ≥1 log"
            ],
            "Value": [
                f"{baseline_hbsag:.2f}", f"{week12_hbsag:.2f}", f"{week24_hbsag:.2f}",
                f"{week12_alt}", f"{display_df['ALT12w_HBsAg12w'].values[0]:.4f}",
                "Yes" if hbsag24wdecline_1 else "No"
            ]
        }))
